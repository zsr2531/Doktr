...
public class CommandLineSwitchProvider : ICommandLineSwitchProvider
{
    ...
    public static CommandLineSwitch Root { get; } = new("Sets the root directory for paths", ".", "-r", "--root");

    public static CommandLineSwitch InputFiles { get; } = new(
        "Sets the paths to .dll and .xml file pairs to generate documentation from. (format: path/to/dll:path/to/xml separate multiple inputs with ';')",
        "", "-if", "--input");

    public static CommandLineSwitch AdditionalIncludes { get; } = new(
        "Sets the paths to copy the contents from to the output directory. (separate paths with ';')", "", "--include");

    public static CommandLineSwitch OutputPath { get; } = new("Sets the output directory path.", "_out", "-o", "--out");

    public static CommandLineSwitch UseTablesForParameters { get; } =
        new("Sets whether the resulting markdown should use tables for parameters.", null, "--use-tables");

    public static CommandLineSwitch XrefUrls { get; } =
        new("Sets the URLs to retrieve external references from. (separate URLs with ';')", "", "--external-xref");

    public static CommandLineSwitch GenerateExample { get; } =
        new("Generates an example config to 'example.xml'.", null, "--generate-example");

    public static CommandLineSwitch Verbose { get; } = new("Shows more output.", null, "--verbose");

    public static CommandLineSwitch Help { get; } = new("Shows this help message.", null, "-h", "--help");

    public static CommandLineSwitch About { get; } =
    ...
    public ImmutableArray<CommandLineSwitch> AllSwitches { get; }

    public ImmutableDictionary<string, CommandLineSwitch> Flags { get; }

    public ImmutableDictionary<string, CommandLineSwitch> Options { get; }
}