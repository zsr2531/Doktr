...
public static class Program
{
    ...
    private static async Task Run(CommandLineParseResult cli)
    {
    ...
    private static async Task RunConfiguration(DoktrConfiguration configuration, IServiceProvider provider)
    {
        ...
        foreach (var target in configuration.InputFiles)
        {
            ...
            var parser = provider.GetRequiredService<IXmlParserService>();
            var result = parser.ParseXmlFile(await File.ReadAllTextAsync(path));
            xmldocBuilder.AddRange(result);
        }

        var xmldoc = xmldocBuilder.ToImmutable();
